 ...
 * (default is all corners) width: width of the effect; in the case of rounded
 * corners this is the radius. specify this value using the px suffix such as
 * 10px (yes, it must be pixels).
 */
;
(function($) {
	...
	var style = document.createElement('div').style, moz = style['MozBorderRadius'] !== undefined, webkit = style['WebkitBorderRadius'] !== undefined, radius = style['borderRadius'] !== undefined
			|| style['BorderRadius'] !== undefined, mode = document.documentMode || 0, noBottomFold = msie
			&& (!mode || mode < 8),
		...
		return this
				.each(function(index) {
					...
					sc = ((o.match(/sc:(#[0-9a-f]+)/) || [])[1]), // strip
																	// color
					width = parseInt((o.match(/(\d+)px/) || [])[1], 10) || 10, // corner
																				// width
					re = /round|bevelfold|bevel|notch|bite|cool|sharp|slide|jut|curl|tear|fray|wicked|sculpt|long|dog3|dog2|dogfold|dog|invsteep|steep/, fx = ((o
